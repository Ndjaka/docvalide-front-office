const computePrice = (
  ...
  // ayos éssé soa obala endom mengang vers mbalmayo
    ...
    residence?.match(/^ayos|éssé|obala|mengang|okola$/i) &&
    tribunal?.match(/^nyong et so'o$/i)
  ) {
    return 7500;
  } else if (
    ...
    residence?.match(
      /^ayos|éssé|monatélé|obala|mengang|okola|akonolinga|mfou|ntui|ngoumou|bafia$/i
    ) &&
  ...
  /*tous les departements de la region du CENTRE sauf à bafia
   *autrement dit le client vie dans la region de CENTRE
   *est né dans le departement de bafia
   *mais vit dans un autre departement
   *exclu yaoundé
   **/
    residence?.match(
      /^monatélé|akonolinga|ntui|ngoumou|mfou|mbalmayo|nanga éboko$/i
    ) &&
    tribunal?.match(/^mbam et inoubou$/i)
  ) {
    return 7500;
  ...
  // aYos soa éssé mengang okoloa endom vers bafia
    ...
    residence?.match(/^ayos|soa|éséka|mengang|okola$/i) &&
    tribunal?.match(/^mbam et inoubou$/i)
  ) {
    return 8000;
  } else if (
  ...
  /*tous les departements de la region du CENTRE sauf à eseka
   *autrement dit le client vie dans la region de CENTRE
   *est né dans le departement de eseka
   *mais vit dans un autre departement
   **/
    residence?.match(
      /^ntui|mfou|ngoumou|monatélé|mbalmayo|nanga éboko|bafia$/i
    ) &&
    tribunal?.match(/^nyong et ékélé$/i)
  ) {
    return 8500;
  ...
  /*tous les departements de la region du CENTRE sauf à monatele
   *autrement dit le client vie dans la region de CENTRE
   *est né dans le departement de monatele
   *mais vit dans un autre departement
   *retiré yaoundé
   **/
  ...
  //ayos essé megang endom obala okola vers monatélé
    ...
    residence?.match(/^ayos|éssé|mengang|obala|okola$/i) &&
    tribunal?.match(/^lékié$/i)
  ) {
    return 7500;
  ...
  // des autres regions vers les departements de la region du centre sauf yaoundé(mfoundi)
    ...
    residence?.match(
      /^édéa|nkongsamba|yabassi|ambam|ébolowa|kribi|sangmelima|bafoussam|bandjoun|bafang|baham|foumban|dschang|mbouda|banganté#/i
    ) &&
    tribunal?.match(
  ...
  // vers la->region du littoral                       // ON DOIT RETIRER LA REGION DE L'OUEST QUI SE TROUVE PROCHE DE LA REGION DU LITTORAL
    ...
    residence?.match(/^soa|endom|ayos|obala|akonolinga|ntui|ngoumou$/i) &&
    tribunal?.match(/^wouri|sanaga maritime|nkam|moungo|mbanga$/i)
  ) {
    return 8500;
  } else if (
  ...
  // des autreS regions vers la region du sud sauf akonolinga
    ...
    residence?.match(
      /^mfou|soa|mengang|endom|ayos|obala|ntui|bandjoun|bafang|baham|foumban|dschang|mbouda|banganténgoumou|mbalmayo|nanga éboko|bafia|éséka|bafoussam|monatélé$/i
    ) &&
